...
const install = (Vue, vm) => {
    ...
    Vue.prototype.$u.http.interceptor.request = config => {
        const noToken = ['api/getCode', 'api/user/login', 'api/user/register', 'api/user/retrieve', 'api/config'];
    ...
    Vue.prototype.$u.http.interceptor.response = res => {
        ...
        if (res.code == 20000) {
            ...
            if (curRoute != 'pages/login/login' && curRoute != 'pages/login/register' && curRoute != 'pages/login/retrieve') {
                vm.$u.toast(res.message, 3000);
                setTimeout(() => {
                    vm.$u.route({ url: '/pages/login/login', type: 'reLaunch' });
                }, 1000);
            }
            return false;
            ...
            vm.$u.toast('系统错误 错误代码：' + res.data.code, 5000);
            return false;
        }
    };
};
